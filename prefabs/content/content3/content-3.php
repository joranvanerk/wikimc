  ...
  <div class="container-fluid" style="padding-top: 5%; padding-left: 12.5%; padding-right: 12.5%; padding-bottom: 1%">
    <div class="card bg-dark">
      ...
      <!-- de card titel van de database -->
        <?php
            ...
            $sql = "SELECT titel from teksten WHERE id='3'";
    ...
    <!-- de card tekst uit de database -->
      <?php
        include("./database/connect_db.php");
        mysqli_query($conn, $sql);
        $sql = "SELECT tekst from teksten WHERE id='3'";
        $results = $conn-> query($sql);
        if ($results->num_rows > 0) {
          while ($row = $results-> fetch_assoc())
        {
    ...
    <?php
        include("./database/connect_db.php");
        mysqli_query($conn, $sql);
        $sql = "SELECT plaatje from teksten WHERE id='3'";
        $results = $conn-> query($sql);
        if ($results->num_rows > 0) {
          while ($row = $results-> fetch_assoc())
        {
...
<div class="container-fluid" style="padding-left: 12.5%; padding-right: 12.5%; padding-bottom: 4%">
  <div class="card bg-dark">
    <div class="card-body">
      ...
      <iframe src="https://online.samensocial.nl/prefabs/content/content3/zenden.php" frameborder="0" height="240" width="1000"></iframe>
        <!-- de ingevoerde comments laden -->
        <?php
          include("./database/connect_db.php");
          mysqli_query($conn, $sql);
          $sql = "SELECT naam, bericht from comments WHERE herken='3'";
          $results = $conn-> query($sql);
          if ($results->num_rows > 0) {
            while ($row = $results-> fetch_assoc())
          {
